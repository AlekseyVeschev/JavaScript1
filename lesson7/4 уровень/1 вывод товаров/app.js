...
function clickHandler(event) {
    ...
    buttons.forEach(button => {
        if (button.dataset.type === event.currentTarget.dataset.type) {
    ...
    for (const product in products) {
        if (product === category)
            for (let i = 0; i < products[product].length; i++) {
                getProductMarkup(products[product][i])
            }
    }
}
...
function getProductMarkup(product) {
    ...
    productsDiv.insertAdjacentHTML("afterbegin",
        ...
        <a href="https://example.com/producs/${product.imageUrl}">Подробнее</a>`
    )
}
