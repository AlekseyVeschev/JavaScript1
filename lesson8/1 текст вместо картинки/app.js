...
function handleClick(clickedButtonEvent) {
    ...
    const targetBtn = clickedButtonEvent.target
    const cardNode = targetBtn.parentElement
...
function hideMoreText(card) {
    ...
    card.img.setAttribute("style", "display: block")
    // 5.1 внутри .product находим элемент с классом .desc и удаляем его
    const divDesc = card.wrap.querySelector(".desc")
    divDesc.remove()
...
function showMoreText(card) {
    ...
    div.innerText = text
    card.productName.insertAdjacentElement("afterend", div)
